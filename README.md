...
### 3. Backend indítása (külön terminálban)
...
1. **frontend** — `npm ci`, `npm run build`, `CI=true npm test` (Node 20 + 22 matrix)
2. **backend** — `npm ci` a `server/`-ben, majd `npm test` (Node 22)
